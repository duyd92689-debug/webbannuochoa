...
@section('page_title', 'Cập nhật sản phẩm: ' . $product->name)
            ...
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    @php
                        ...
                        $brandList = $brands ?? [];
                        // Nếu product có brand nhưng chưa có trong list thì bổ sung vào list
                        if ($selectedBrand && !in_array($selectedBrand, $brandList)) {
                    ...
                    <div class="col-md-6 form-group">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            ...
                            <button type="button" class="btn btn-link btn-sm p-0 text-decoration-none" id="toggleBrandBtn" style="font-size:0.83rem;">
                                <i class="fa-solid fa-pen-to-square mr-1"></i><span id="toggleBrandText">Nhập hãng mới</span>
                            </button>
                        </div>

                        {{-- Menu chọn thương hiệu sổ xuống --}}
                        <div id="brandSelectWrap">
                            <select id="brand_select" name="brand" class="form-control" required>
                                ...
                                @foreach ($brandList as $b)
                                    <option value="{{ $b }}" {{ strcasecmp($selectedBrand, $b) === 0 ? 'selected' : '' }}>
                                        {{ $b }}
                                    </option>
                                @endforeach
                        ...
                        <div id="brandInputWrap" style="display: none;">
                            <div class="input-group">
                                <input type="text" id="brand_custom" class="form-control" placeholder="Nhập tên thương hiệu mới (VD: Le Labo, Roja...)" maxlength="120">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        <select name="gender" id="gender" class="form-control" required>
                            ...
                            <option value="nu" {{ old('gender', $product->gender) == 'nu' ? 'selected' : '' }}>Nữ</option>
                            <option value="unisex" {{ old('gender', $product->gender) == 'unisex' ? 'selected' : '' }}>Unisex</option>
                        </select>
                    </div>
                    <div class="col-md-4 form-group">
                        <label for="concentration" class="font-weight-bold" style="font-size:0.9rem;">Nồng độ</label>
                        <input type="text" name="concentration" id="concentration" class="form-control" value="{{ old('concentration', $product->concentration) }}">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-3 form-group">
                        ...
                        <input type="number" name="volume_ml" id="volume_ml" class="form-control" value="{{ old('volume_ml', $product->volume_ml) }}" min="1" required>
                        ...
                        <div class="mt-2 d-flex flex-wrap gap-1" style="gap: 4px;">
                            ...
                            <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2 quick-volume" data-val="100">100ml</button>
                    ...
                    <div class="col-md-3 form-group">
                        ...
                        <div class="input-group">
                            <input type="number" name="weight" id="weight" class="form-control" value="{{ old('weight', $product->weight ?? 200) }}" min="1" max="50000" required placeholder="200">
                            <div class="input-group-append">
                        ...
                        <div class="mt-2 d-flex flex-wrap gap-1" style="gap: 4px;">
                            ...
                            <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2 quick-weight" data-val="200">200g</button>
                    ...
                    <div class="col-md-3 form-group">
                        ...
                        <input type="number" name="sale_price" id="sale_price" class="form-control" value="{{ old('sale_price', $product->sale_price) }}" min="0">
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        <input type="number" name="stock_10ml" id="stock_10ml" class="form-control" value="{{ old('stock_10ml', $product->stock_10ml) }}" min="0" placeholder="Tự tính">
                    </div>
                    <div class="col-md-4 form-group">
                        <label for="stock_50ml" class="font-weight-bold" style="font-size:0.9rem;">Kho Chai 50ml</label>
                        <input type="number" name="stock_50ml" id="stock_50ml" class="form-control" value="{{ old('stock_50ml', $product->stock_50ml) }}" min="0" placeholder="Tự tính">
                    </div>
                </div>

                <div class="row">
                    <div class="col-md-6 form-group">
                        <label for="image_url" class="font-weight-bold" style="font-size:0.9rem;">Đường dẫn ảnh</label>
                        <input type="text" name="image_url" id="image_url" class="form-control" value="{{ old('image_url', $product->image_url) }}">
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="image_file" class="font-weight-bold" style="font-size:0.9rem;">Thay ảnh mới (tệp tin)</label>
                ...
                <div class="form-group">
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" name="is_active" id="is_active" class="custom-control-input" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }}>
                        <label class="custom-control-label font-weight-bold" for="is_active">Kích hoạt hiển thị sản phẩm trên cửa hàng</label>
                    </div>
                </div>
