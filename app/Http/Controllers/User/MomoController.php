...
{
    public function start(Order $order, MomoService $momo, Request $request)
    {
        ...
        return $this->redirectToMomo($order, $this->newTransaction($order), $momo, $requestType);
    }

    public function payAgain(Order $order, MomoService $momo, Request $request)
    {
        if ($order->user_id !== Auth::id()) {
            abort(403);
        }
    ...
    public function showQr(Order $order, MomoService $momo)
    {
        if ($order->user_id !== Auth::id()) {
            abort(403);
        }
    ...
    private function redirectToMomo(Order $order, PaymentTransaction $transaction, MomoService $momo, string $requestType = 'payWithATM')
    {
